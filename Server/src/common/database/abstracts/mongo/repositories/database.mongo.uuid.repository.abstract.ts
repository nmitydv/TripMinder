...
> extends DatabaseBaseRepositoryAbstract<EntityDocument> {
    ...
    async findAllByUserId<T = EntityDocument>(
        find?:  Record<string, any> ,
        options?: IDatabaseFindAllOptions<ClientSession>
    ): Promise<T[]> {
        const findAllByUserId = this._repository.find<EntityDocument>(find);
        ...
        if (options?.withDeleted) {
            ...
            findAllByUserId.where(DATABASE_DELETED_AT_FIELD_NAME).exists(false);
        }

        if (options?.select) {
            findAllByUserId.select(options.select);
        }

        if (options?.paging) {
            findAllByUserId.limit(options.paging.limit).skip(options.paging.offset);
        }

        if (options?.order) {
            findAllByUserId.sort(options.order);
        }

        if (options?.join) {
            findAllByUserId.populate(
        ...
        if (options?.session) {
            findAllByUserId.session(options.session);
        }


        return findAllByUserId.lean() as any;
    }
    ...
    async findByEmail<T = EntityDocument>(
        email: string,
        options?: IDatabaseFindOneOptions<ClientSession>
    ): Promise<T> {
        const findOne = this._repository.findOne<EntityDocument>({ email: email });
