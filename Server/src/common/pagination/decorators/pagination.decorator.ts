...
import { PaginationFilterInEqualPipe } from '../pipes/pagination.filter-in-equal.pipe';
import { PaginationFilterInEqualNumberPipe } from '../pipes/pagination.filter-equal-in-number';
import { PaginationFilterRangePipe } from '../pipes/pagination.filter-range.pipe';
...
export function PaginationQueryFilterInEqual(
    field: string,
    defaultValue: string[]
): ParameterDecorator {
    return Query(field, PaginationFilterInEqualPipe(defaultValue));
}

export function PaginationQueryFilterInEqualnumber(
    field: string,
    defaultValue: number[]
): ParameterDecorator {
    return Query(field, PaginationFilterInEqualNumberPipe(defaultValue));
}

export function PaginationQueryFilterInRange(
    field: string,
    defaultValue: number[]
): ParameterDecorator {
    return Query(field, PaginationFilterRangePipe(defaultValue));
}